    ...
    <div>
        ...
        <div class="flex flex-row space-x-2 md:space-x-3 lg:space-x-10 font-default">
            ...
            <div class="basis-1/3 md:space-y-3 p-2">
                ...
                <div class="text-lg">
                    <p class="md:text-xl">Brand: {{ brand?.name }}</p>
                    <p class="md:text-xl">Pieces: {{ item.pieces }}</p>
                    <p class="md:text-xl">Unit Price: C${{ item.unitPrice }}</p>
                </div>
                
                <div class="flex flex-row md:space-x-2 items-center text-lg">
                    <div class="join">
                        <button class="btn btn-sm rounded-r-sm" @click="onClickMinus()">-</button>
                        <input type="text" class="input input-bordered input-sm text-lg w-[3rem] rounded-none text-center" :input="quantity" :value="latestQnt"/>
                        <button class="btn btn-sm rounded-l-sm" @click="onClickPlus()">+</button>
                    </div>
                    <div class="btn btn-link text-black">
                        <trash-icon @click="() => quantity = 0" class="w-6 h-6"/>
                    </div>
